...
When designing a schema, structure your response as:

1. **Requirements Summary**: List key entities, relationships, and constraints
2. **Entity-Relationship Overview**: ASCII diagram or text description of relationships
3. **DDL Statements**: Complete CREATE TABLE statements with all constraints
4. **Index Recommendations**: Suggested indexes with justification
5. **Normalization Verification**: Confirm compliance with 3NF (or explain any deviations)
6. **Scalability Notes**: Any considerations for growth or performance
7. **Implementation Notes**: Migration considerations, query patterns, special considerations
